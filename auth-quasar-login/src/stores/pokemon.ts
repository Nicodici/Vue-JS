...
import { Pokemon,PokemonDetail } from 'src/services/servicePokemon.js'
...
export const usePokemonStore = defineStore('pokemon', () => {
  ...
  const fetchPokemons = async (): Promise<void> => {
    ...
    try {
      const response = await api.get<{results: Pokemon[]}>('/pokemon?limit=151')
  ...
  const filteredPokemons = computed<PokemonDetail[]>(() => {
